...
void my_exit_group(int status)
{
	...
	//we don't care if this returns -1 or not
        //the pid no longer in the list, regardless of the return value
	del_pid( pid );
...
int validate_sysc(int sysc){
    if(sysc > 0 && sysc != MY_CUSTOM_SYSCALL && sysc <= NR_syscalls){
        return 1;
    }
    return -1;
}
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    switch (table[sysc].monitored){
        case( 1 ):
            if( check_pid_monitored( sysc, pid ) > 0 ){
                log_message( pid, reg.ax, reg.bx, reg.cx, reg.dx, 
    ...
    spin_unlock( &pidlist_lock );
    //According to Anish Patel this is safe to do
    res = table[sysc].f(reg);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    //Find out what system call we're to do
    //  - REQUEST_SYSCALL_INTERCEPT to intercept the 'syscall' argument
    //  - REQUEST_SYSCALL_RELEASE to de-intercept the 'syscall' argument
    //  - REQUEST_STOP_MONITORING to stop monitoring for 'pid'
    //  - REQUEST_START_MONITORING to start monitoring for 'pid' whenever it issues 'syscall' 
    ...
    switch(cmd){
        ...
        case REQUEST_START_MONITORING:
            //Check permissions : root must specify 0 or a valid pid
            //                    user must own specified pid
            //Also check if pid is a valid pid.
            if(pid < 0){
                return -EINVAL;
            }
            
            if(current_uid() != 0){
                //make sure the pid is owned by the user + pid != 0
                if(pid == 0 || check_pid_from_list((pid_t)pid, current->pid) !=0){
                    return -EPERM;
                }
            }else{   
            ...
            if(validate_sysc(syscall) == 1){
                ...
                //if monitoring single pid
                }else{
                    if(table[syscall].monitored == 2 || check_pid_monitored(syscall, (pid_t) pid)){
                        res = -EBUSY;
                    }else{
                        res = add_pid_sysc((pid_t) pid, syscall);
        ...
        case REQUEST_STOP_MONITORING:
            ...
            if(validate_sysc(syscall) == 1){
                spin_lock( &pidlist_lock );
                //if want to stop monitoring all
                if(pid == 0){
                    if(table[syscall].monitored != 2 ){
                        res = -EINVAL;
                    }else{
                       if(table[syscall].listcount){
                            table[syscall].monitored = 1;
                        }else{
                ...
                //if monitoring single pid
                    ...
                    if(check_pid_monitored(syscall, (pid_t) pid) && table[syscall].monitored == 1 ){
                        res = del_pid_sysc((pid_t) pid, syscall);
